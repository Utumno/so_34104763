...
class ExampleWidget(QtGui.QWidget):
    ...
    def get_hierarchy(self):
        print "nope"
        global HIERARCHY
        HIERARCHY = self.get_nodes_hierarchy( root=self.treeWidget.invisibleRootItem() )
    ...
    def highlight_instances(self):
        ...
        uids = [ getattr(x.person_data, "uid") for x in self.treeWidget.selectedItems() if hasattr(x.person_data, "uid")]
        INSTANCED_NODES = []
        for uid in uids:
            _instances = self.get_instanced_nodes( root=self.treeWidget.invisibleRootItem(), uids=[uid] )
            if len(_instances) > 1:
